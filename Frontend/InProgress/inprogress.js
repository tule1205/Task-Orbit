...
function isInProgressStatus(status) {
  ...
  return s === 'in progress' || s === 'in_progress' || s.includes('progress');
}

async function fetchSubtasksInProgress() {
  ...
  const rows = data || [];
  // Debug helper: see what statuses exist when nothing is shown
  if (rows.length === 0) {
    console.log('No sub_tasks found at all for In Progress view.');
  } else {
    console.log('Sub_tasks fetched for In Progress (id, title, status):', rows.map((s) => ({
      id: s.id,
      title: s.title,
      status: s.status,
    })));
  ...
  // Treat "in progress" (any casing/spacing) and todo as visible here
  const visible = rows.filter((s) => {
    const status = s.status == null ? '' : String(s.status).toLowerCase().replace(/\s+/g, ' ').trim();
    return !status || status === 'todo' || isInProgressStatus(status);
  });

  const inProgress = visible.slice().sort((a, b) => {
  ...
  if (rows.length > 0 && inProgress.length === 0) {
    console.log('Sub_tasks in DB (status values):', rows.map((s) => ({ id: s.id, title: s.title, status: s.status })));
  }
  return inProgress;
}
...
async function deleteSubtask(id) {
  ...
  const list = await fetchSubtasksInProgress();
  ...
  if (list.length > 0 && selectedSubtaskId && list.some((s) => s.id === selectedSubtaskId)) {
    ...
    getEl('subtaskDescription').textContent = 'No subtasks in progress.';
  }
}
...
async function selectSubtask(sub) {
  ...
  if (!description && sub.task_id) {
    const { data: task } = await supabase.from('tasks').select('description').eq('id', sub.task_id).single();
    description = task?.description || 'No description.';
  }
  getEl('subtaskDescription').textContent = description || 'No description.';
  ...
  pill.addEventListener('drop', async (e) => {
    ...
    if (!error) {
      ...
      if (String(selectedSubtaskId) === String(id)) {
        ...
        getEl('subtaskTitle').textContent = '—';
        getEl('subtaskDescription').textContent = 'No subtasks selected.';
...
async function init() {
  ...
  if (list.length > 0 && !selectedSubtaskId) {
    ...
    getEl('subtaskTitle').textContent = '—';
    getEl('subtaskDescription').textContent = 'No subtasks in progress. Move subtasks to In Progress from the dashboard.';
  }
}
